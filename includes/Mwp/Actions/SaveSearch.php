...
class Mwp_Actions_SaveSearch{
	...
	public function enqueue_scripts(){
		wp_enqueue_script( 
			$this->plugin_name . '-save-search', 
			mwp_public_url() . 'js/save-search-min.js', 
			array( 'jquery' ), 
			$this->version, 
			true 
		);
	...
	private function _get_current_location_name($search_data){
		...
		if( isset($search_data['city']) && ($search_data['cityid'] != 0 || $search_data['city'] != '') ){
			$str_save_location = $search_data['city'];
		}
		if( isset($search_data['community']) && ($search_data['communityid'] != 0 || $search_data['community'] != '') ){
			$str_save_location = $search_data['community'];
		}
		if( isset($search_data['subdivision']) && ($search_data['subdivisionid'] != 0 || $search_data['subdivision'] != '') ){
	...
	private function _get_property_type($search_data){
		...
		$hook_property_type = apply_filters('fields_type_' . mwp_get_current_api_source(), $hook_property_type);
	...
	private function _get_price($search_data){
		...
		if( $min_price > 0 || $max_price > 0 ){
			...
			$price = ', '. __('Price', mwp_localize_domain()).' : ' . $get_currency.number_format($min_price,2) . ' - ' . $get_currency.number_format($max_price);
		}
		return $price;
	}
	...
	private function _get_bed($search_data){
		...
		if( $min_bed > 0 || $max_bed > 0 ){
			$bed = ', '. __('Bed', mwp_localize_domain()).' ' . $min_bed . ' - ' . $max_bed;
		}
		return $bed;
	}
	...
	private function _get_bath($search_data){
		...
		if( $min_baths > 0 || $max_baths > 0 ){
			$baths = ', '. __('Baths', mwp_localize_domain()).' ' . $min_baths . ' - ' . $max_baths;
		}
		return $baths;
	}
	...
	public function save_search_init($data = array()){
		...
		$transaction = $ajax_data_post['transaction'];
		if( is_array($transaction) && count($transaction) > 0 ){
			$transaction = implode(", ", $ajax_data_post['transaction']);
		}
		...
		if( trim($ajax_data_post['mls']) != '' && mwp_get_current_api_source() == 'mls' && $location_name != '' ){
			$property_alert = Mwp_Propertyalert_Entity::get_instance()->subscribe_property_alert($ajax_data_post);
	...
	public function display_button($atts){
		...
		$is_community 	= false;
		if(
			(isset($_GET['communityid'])  && $_GET['communityid'] != '0')
			||
			(isset($_REQUEST['communityid']) && $_REQUEST['communityid'] != '0')
		)
		{
			$is_community 	= true;
			$communityid = '0';
		...
		$is_city 	= false;
		if(
			(isset($_GET['cityid']) && $_GET['cityid'] != '0')
			||
		...
		if(
			(isset($_GET['subdivisionid']) && $_GET['subdivisionid'] != '0')
			||
			(isset($_REQUEST['subdivisionid']) && $_REQUEST['subdivisionid'] != '0')
		)
		{
			$subdivisionid = '0';
		...
		$is_county = false;
		if(
			(isset($_GET['countyid']) && $_GET['countyid'] != '' && $_GET['countyid'] != '0')
			||
			(isset($_REQUEST['countyid']) && $_REQUEST['countyid'] != '' && $_REQUEST['countyid'] != '0')
		)
		{
		...
		$content = __('Need to login or register before saving this search', mwp_localize_domain());
		...
		$btn_class = 'btn-primary-savesearch';
		if( $save_search ){
			$btn_class = 'btn-success-savesearch';
		}

		$btn_name = __('Save This Search', mwp_localize_domain());
		if( $save_search ){
			$btn_name = __('Saved Search', mwp_localize_domain());
		}
		...
		if( trim($location_name) != '' ){
			...
			$data['save_search_name'] = $save_search_name;
			$data['btn_icon'] = $btn_icon;
			$data['btn_name'] = $btn_name;
			$data['content'] = $content;
			$template = Mwp_Theme_Locator::get_instance()->locate_template('save-search/save-search-button.php');
