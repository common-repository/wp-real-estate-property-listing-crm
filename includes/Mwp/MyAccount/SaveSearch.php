<?php
class Mwp_MyAccount_SaveSearch extends Mwp_MyAccount_Dashboard{
		...
		switch($task){
			case 'update_save_search':
				if( isset($_POST['save_search_name']) ){
					...
					foreach($post_umeta_key as $key => $val){
						$get_umeta_key = Mwp_Actions_SaveSearch::get_instance()->get_save_search($key,true);
						$get_umeta_key['save_search_name'] = $val;
						update_user_meta( $user_account->ID, $get_umeta_key['user_meta_name'], $get_umeta_key );
					}
				}
				Mwp_Helpers_File::redirect_to($redirect);
			break;
			case 'subscribe':
				...
				if(wp_verify_nonce( $nonce, $task.'-saved-search-'. $key . $user_account->ID ) ){
					...
					if( $umeta_key ){
						$p_alert = Mwp_Propertyalert_Entity::get_instance()->subscribe_property_alert($umeta_key);
						if($p_alert->result == 'success'){
							if( !isset($umeta_key['subscribed_property_alert']) ){
								...
								if( update_user_meta( $user_account->ID, $umeta_key['user_meta_name'], $umeta_key ) ){
									Mwp_Actions_SaveSearch::get_instance()->increment_search_counter($umeta_key['md5_save_search_name']);
								}
							}
						}
					}
			...
			case 'unsubscribe-all':
				if(wp_verify_nonce($nonce, 'un-subscribe-all-' . $user_account->ID) ){
					$unsubscribe = Mwp_Propertyalert_Entity::get_instance()->crm_unsubscribe($user_account->user_email);
				}
				Mwp_Helpers_File::redirect_to($redirect);
			break;
			case 'trash':
				...
				if(wp_verify_nonce($nonce, 'trash-saved-search-'. $key . $user_account->ID) ){
					...
					if( $umeta_key ){
						if( delete_user_meta($user_account->ID, 'save-search-'.$key) ){
							Mwp_Actions_SaveSearch::get_instance()->decrement_search_counter($key);
							Mwp_Helpers_File::redirect_to($redirect);
						}
					}
				}
	...
	public function display_save_search(){
		...
		$template = Mwp_Theme_Locator::get_instance()->locate_template('account/save-search-list.php');
		$data['db_save_search'] = $db_save_search;
		$data['user_account'] = $user_account;
		$data['has_saved_search'] = $has_saved_search;
