<?php
class Mwp_MyAccount_Profile extends Mwp_MyAccount_Dashboard{
	...
	public function enqueue_scripts(){
		$dashboard_page = Mwp_MyAccount_Dashboard::get_instance()->get_dashboard_page();
		if( $dashboard_page && is_page($dashboard_page->ID) ){
			wp_enqueue_script(
				$this->plugin_name . '-profile-actions', 
				mwp_public_url() . 'js/account-min.js', 
				array( 'jquery' ), 
				$this->version, 
				true 
			);
	...
	public function template($template = null){
		if( is_null($template) ){
			$template = Mwp_Theme_Locator::get_instance()->locate_template('account/profileform.php');
		}
	...
	public function get_password_form($template = null){
		if( is_null($template) ){
			$template = Mwp_Theme_Locator::get_instance()->locate_template('account/passwordform.php');
		}
		// hook filter, incase we want to just use hook
	...
	public function update_password_callback(){
		...
		if(
			...
			$msg 	= __('Please Confirm password', mwp_localize_domain());
			$status = false;
		}else{
			wp_set_password( $new_password, $user_id );
			$msg 	= __('Successfully Change Password', mwp_localize_domain());
			$status = true;
		}
