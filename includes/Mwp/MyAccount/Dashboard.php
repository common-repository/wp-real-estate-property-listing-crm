...
class Mwp_MyAccount_Dashboard {
	...
	public function md_set_query_var($query_vars){
		...
		$this->md_query_vars = Mwp_Helpers_Text::array_to_object($md_query);
	}
	...
	public function navigation($data = array()){
		$data['query'] = $this->md_get_query_vars();
		$data['action'] = $data['query']->action;
		$nav = Mwp_Theme_Locator::get_instance()->locate_template('account/nav.php');
		Mwp_View::get_instance()->display($nav, $data);
	}

	public function template(){
		return Mwp_Theme_Locator::get_instance()->locate_template('account/main.php');
	}

	public function content($ret = false, $data = array()){
		if( $ret ){
	...
	public function subscriber_modify_wp_admin_bar(){
		...
		$wp_admin_bar->add_node( array(
			'id' => 'my-account',
			'href'=> Mwp_MyAccount_Profile::get_instance()->url(),
		));
        $wp_admin_bar->add_menu( array(
			...
			'title' => __('Edit Profile', mwp_localize_domain()), // link title
			'href' => Mwp_MyAccount_Profile::get_instance()->url(), // name of file
			'meta' => false // array of any of the following options: array( 'html' => '', 'class' => '', 'onclick' => '', target => '', title => '' );
		));
		$wp_admin_bar->add_menu( array(
            'parent'=> 'user-actions',
            'id'    => 'md_account_logout',
            'title' => __('Log Out', mwp_localize_domain()),
            'href'  => wp_logout_url()
        ));
	}
	...
	public function modify_wp_admin_bar(){
        ...
        $wp_admin_bar->add_menu( array(
			...
			'title' => __('Favorites', mwp_localize_domain()), // link title
			'href' => Mwp_MyAccount_Favorite::get_instance()->url(), // name of file
			'meta' => false // array of any of the following options: array( 'html' => '', 'class' => '', 'onclick' => '', target => '', title => '' );
		));
        ...
        $wp_admin_bar->add_menu( array(
			...
			'title' => __('X-Out',mwp_localize_domain()), // link title
			'href' => Mwp_MyAccount_Xout::get_instance()->url(), // name of file
			'meta' => false // array of any of the following options: array( 'html' => '', 'class' => '', 'onclick' => '', target => '', title => '' );
		));
        ...
        $wp_admin_bar->add_menu( array(
			...
			'title' => __('Save Search', mwp_localize_domain()), // link title
			'href' => Mwp_MyAccount_SaveSearch::get_instance()->url(), // name of file
			'meta' => false // array of any of the following options: array( 'html' => '', 'class' => '', 'onclick' => '', target => '', title => '' );
		));
